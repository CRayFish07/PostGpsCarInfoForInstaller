package com.example.postgpscarinfoforinstaller.net;
...
import com.example.postgpscarinfoforinstaller.Flag;
import com.example.postgpscarinfoforinstaller.util.AppUtil;
...
//	public final String SVR_URL = "http://211.156.198.50/";
	...
	final String GATEWAYLOG="gatewayLog";
	...
	final String REDISLOG4APP="redisLog4App";
...
//			rmap.put("data", "");
	...
	public Map<String, Object> redisLog4App(String c_cph,String c_sbxlh){
		Log.i(TAG, "调用接口，iface:" + GATEWAYLOG + "!"
				+ REDISLOG4APP+"参数：cph="+c_cph+"c_sbxlh="+c_sbxlh);
		Map<String, Object> rmap;
		JSONObject o = new JSONObject();
		try {
			o.put("c_cph", c_cph);
			o.put("c_sbxlh", c_sbxlh);
			rmap = netHelper.req(GATEWAYLOG, REDISLOG4APP,
					o.toString(), null);
			...
			rmap.put("result", -3);
			rmap.put("msg", "接口参数解析异常");
			Log.e(TAG, "接口参数解析异常，iface:" + GATEWAYLOG + "!"
					+ REDISLOG4APP + ",c_sbxlh:" + c_sbxlh+"cph:"+c_cph);
		}
		return rmap;
	}
}
